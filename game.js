...
function discover(next){let r=S.team.length>1?rel(...S.team):null;if(r&&r.type==='legendary'){overlay.innerHTML=`<div class="overlay unlock-overlay"><div class="discover unlock-discover"><p>LEGENDARY TEAM DISCOVERED</p><div class="actions top-actions"><button class="btn" id="continue">CONTINUE</button></div><div class="pair unlock-pair">${card(S.team[0])}${card(S.team[1])}</div><h1>${r.teamName}</h1></div></div>`;document.getElementById('continue').onclick=()=>{overlay.innerHTML='';next()}}else next()}
function team(){clearStoryTimer();render(`<section class="panel"><h1 class="title">Your Team</h1><div class="actions top-actions"><button class="btn" onclick="opponent()">FIND OPPONENT</button></div><p class="sub">${rel(...S.team)?.teamName||S.team.map(x=>x.name).join(' & ')}</p><div class="cards two">${S.team.map(x=>card(x)).join('')}</div></section>`)}
function opponent(){let ids=new Set(S.team.map(x=>x.id)),eligible=WRESTLERS.filter(x=>!ids.has(x.id));S.opp=pick(eligible,2);render(`<section class="panel"><h1 class="title">Next Match</h1><div class="actions top-actions"><button class="btn" onclick="match()">START MATCH</button></div><div class="battle"><div><div class="cards">${S.team.map(x=>card(x)).join('')}</div></div><div class="vs">VS</div><div><div class="cards">${S.opp.map(x=>card(x)).join('')}</div></div></div></section>`)}
...
function acceptSinglesChallenge(index){
 ...
 render(`<section class="panel singles-intro"><small>FEATURED SINGLES MATCH</small><div class="actions top-actions"><button class="btn" onclick="match()">BEGIN SINGLES MATCH</button></div><h1>${chosen.name}</h1><div class="singles-vs">VS</div><h1>${challenger.name}</h1><p>${challenger.title} issued the challenge. ${chosen.title} accepted it.</p></section>`);
}
...
function reward(t){if(t==='wrestler'){let ids=new Set(S.team.map(x=>x.id));S.offer=one(WRESTLERS.filter(x=>!ids.has(x.id)));render(`<section class="panel"><h1 class="title">New Wrestler</h1><div class="actions top-actions"><button class="btn" onclick="team()">KEEP TEAM</button><button class="btn" onclick="replace(0)">REPLACE ${S.team[0].name}</button><button class="btn" onclick="replace(1)">REPLACE ${S.team[1].name}</button></div><div style="max-width:340px;margin:auto">${card(S.offer)}</div></section>`)}else if(t==='chem'){S.chem+=5;team()}else if(t==='momentum'){S.momentum+=2;team()}else{S.wind=true;S.windAwarded=true;team()}}
