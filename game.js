...
/* Character Image Developer Mode 1.0
   Open with ?dev=images or press Ctrl+Shift+I. Values are previewed live,
   saved locally for testing, and can be copied into imageManager.js. */
(function initCharacterImageDeveloperMode(){
 const DEV_KEY='ttg_image_manager_preview_v2';
 let panel=null,selectedId=Object.keys(CHARACTER_IMAGE_MANAGER)[0]||'';
 ...
 function apply(id){const c=config(id);if(!c)return;const t=c.transform||{};document.querySelectorAll(`.wrestler-${CSS.escape(id)}.framework-custom`).forEach(img=>{img.style.setProperty('--custom-scale',t.scale??1);img.style.setProperty('--custom-x',`${t.x??0}px`);img.style.setProperty('--custom-y',`${t.y??0}px`)})}
 function applyAll(){Object.keys(CHARACTER_IMAGE_MANAGER).forEach(apply)}
 function loadPreview(){try{const saved=JSON.parse(localStorage.getItem(DEV_KEY)||'{}');Object.entries(saved).forEach(([id,t])=>{if(config(id))config(id).transform={...config(id).transform,...t}})}catch(e){}}
 function savePreview(){const out={};Object.entries(CHARACTER_IMAGE_MANAGER).forEach(([id,c])=>out[id]=c.transform);localStorage.setItem(DEV_KEY,JSON.stringify(out))}
 function snippet(id){const c=config(id),t=c.transform;return `'${id}': {\n  assets: {\n    full: '${c.assets.full}',\n    portrait: '${c.assets.portrait}',\n    victory: '${c.assets.victory}'\n  },\n  transform: { scale: ${Number(t.scale).toFixed(2)}, x: ${Math.round(t.x)}, y: ${Math.round(t.y)} }\n}`}
 function refresh(){if(!panel)return;const c=config(selectedId),t=c.transform;panel.querySelector('[data-scale]').value=t.scale;panel.querySelector('[data-scale-out]').textContent=Number(t.scale).toFixed(2);panel.querySelector('[data-x]').value=t.x;panel.querySelector('[data-x-out]').textContent=Math.round(t.x);panel.querySelector('[data-y]').value=t.y;panel.querySelector('[data-y-out]').textContent=Math.round(t.y)}
 function setValue(key,value){const c=config(selectedId);c.transform[key]=value;apply(selectedId);refresh();savePreview()}
 function open(){
  if(panel){panel.remove();panel=null;return}
  localStorage.setItem('ttg_image_dev_enabled','1');
  panel=document.createElement('aside');panel.className='image-dev-panel';
  panel.innerHTML=`<div class="image-dev-head"><strong>IMAGE MANAGER</strong><button data-close>×</button></div><label>Wrestler<select data-wrestler>${Object.keys(CHARACTER_IMAGE_MANAGER).map(id=>`<option value="${id}">${id}</option>`).join('')}</select></label><label>Scale <output data-scale-out></output><input data-scale type="range" min="0.50" max="2.50" step="0.01"></label><label>X <output data-x-out></output><input data-x type="range" min="-150" max="150" step="1"></label><label>Y <output data-y-out></output><input data-y type="range" min="-180" max="180" step="1"></label><div class="image-dev-actions"><button data-copy>Copy config</button><button data-reset>Reset preview</button></div><small>Preview values persist on this device. Copy config into assets/config/imageManager.js for the permanent build.</small>`;
  ...
  panel.querySelector('[data-copy]').onclick=async e=>{try{await navigator.clipboard.writeText(snippet(selectedId));e.target.textContent='Copied';setTimeout(()=>e.target.textContent='Copy config',900)}catch(_){prompt('Copy this config:',snippet(selectedId))}};
 ...
 if(enabled())setTimeout(open,250);
})();
